...
---
> **Fork notice**: originally installed from [ccheney/robust-skills](https://github.com/ccheney/robust-skills). This version has been locally tuned; check the upstream repo for its license before redistributing.
...
| **Hardcoded Presentation Strings** | User-facing text (toasts, labels, error messages) hardcoded as literals in view models or components | All presentation strings via i18n (e.g. `t('builder.character_limit_reached')`) — never embed language in the adapter |

### Presentation Adapter Invariants

The presentation layer is an adapter. It translates domain signals into UI feedback — but it must not own the words.

**Violation:**
```typescript
// useCharacterForm.ts
toast.error('Free plan: 3 characters max. Add your API key in Settings.')
```

**Fix:**
```typescript
// features/character/presentation/i18n/en.json
{ "builder": { "character_limit_reached": "Free plan: 3 characters max. ..." } }

// useCharacterForm.ts
const { t } = useTranslation('character')
toast.error(t('builder.character_limit_reached'))
```

**Rule:** Any string a user reads → i18n file. Domain errors are caught at the adapter boundary and translated into localized messages; the error kind (e.g. `QuotaExceededError`) is structural, the message is a presentation concern.
